...
def compute_perplexities_for_seq_file(
    ...
    perplexity_fname_regex = r'[a-z\-]+(\d+)-([\d.]+)-(\d+).traindocprobs'
    ...
    for fname in os.listdir(output_dir_name):
        ...
        if perplexity_match is None:
            continue
        f_proc_id, f_prop, f_n_topics = perplexity_match.group(1, 2, 3)
...
def plot_template_perplexities(
        input_dir_name,
        output_dir_name,
        file_prefix,
        save_file,
    ...
    dataf = DataFrame([p for p in perplexities])
    g = sns.FacetGrid(
            dataf,
            col='n_topics',
            row='contents',
            legend_out='true')
    g.map(sns.boxplot, 'proportion', 'perplexity')
...
if __name__ == '__main__':
    ...
    plot_template_perplexities(input_dir_name, output_dir_name, file_prefix, save_file)
