...
def gather_entropies_for_seq_file(
    ...
    for fname in os.listdir(output_dir_name):
        fields = validate_fname(fname, 'lsa.w.txt', file_prefix=seq_file_prefix, n_freqs_list=N_FREQS_LIST)
        if fields is None:
            continue
...
def plot_exact_entropies(
    ...
    for seq_fname in os.listdir(input_dir_name):
        ...
        seq_fields = validate_fname(seq_fname, 'txt', file_prefix=file_prefix, process=process, n_freqs_list=N_FREQS_LIST)
    ...
    g = sns.FacetGrid(
            ...
            row='label',
            legend_out='true')
    g.map(sns.pointplot, 'frequency', 'entropy')
